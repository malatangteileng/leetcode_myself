...
```
递归
二叉排序树的重要性质是，左子树上的节点值一定小于根节点，右子树上的节点值一定大于根节点。  
由此，可知若根节点为ii，则左子树的节点为[1,...,i-1]，右子树的节点为[i+1,...,n]。可递归的构建二叉搜索树。

特判，若n==0，返回[][]

定义生成树函数build\_Trees(left,right)，表示生成[left,...,right]的所有可能二叉搜索树：
